...
class ClearGroup2 extends Command {
	...
	public function fire()
	{
        ...
        foreach($res as $row){
            $ids = array();
            $els = DB::select(DB::raw("SELECT s.id  as id FROM subscribers as s JOIN  subscriber_group as sg on  s.id = sg.subscriber_id and s.email like '".$row->email."%' and sg.group_id in(2) LIMIT 50000"));
            foreach($els as $el)
            {
                $ids[] = $el->id;
                $k++;
